...
import { DaoShieldedTokenSimulator } from "./dao-shielded-token-simulator.js";
import {
  NetworkId,
  setNetworkId
...
describe("DAO Shielded Token smart contract", () => {
  it("generates initial ledger state deterministically", () => {
    const initNonce = new Uint8Array(32).fill(1);
    const simulator0 = new DaoShieldedTokenSimulator(initNonce);
    const simulator1 = new DaoShieldedTokenSimulator(initNonce);
  ...
  it("properly initializes ledger state and private state", () => {
    ...
    const simulator = new DaoShieldedTokenSimulator(initNonce);
  ...
  it("mints DAO voting tokens correctly", () => {
